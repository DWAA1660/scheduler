...
def manage_employee(employer_token_sent, employee_id_sent):
    ...
    return render_template("/employers/manage_employee.html", employee_results_sent=employee_results, employer_results_sent=employer_results)
...
def employermain(token):
    ...
    return render_template("/employers//employerportal.html", your_token=token, name=results["name"], your_id=results["id"], employees=results["employees"], db=db)


@employer_routes.route("/employersignup", methods=['GET'])
def employersignup():
    return render_template('/employers/employer_signup.html')

@employer_routes.route("/employerlogin", methods=['GET'])
def employerlogin():
    return render_template('/employers/employerlogin.html')
